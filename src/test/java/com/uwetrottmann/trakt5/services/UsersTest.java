...
public class UsersTest extends BaseTestCase {
	...
	public void test_collectionMovies() throws IOException {
		List<BaseMovie> movies = executeCall(getTrakt().users().collectionMovies(TestData.USER_SLUG, null));
	...
	public void test_createList() throws IOException {
		...
		// ...and delete it again
		Response deleteResponse = getTrakt().users().deleteList(UserSlug.ME, String.valueOf(createdList.ids.trakt)).execute();
	...
	public void test_updateList() throws IOException {
		...
		// create list...
		TraktList updatedList = executeCall(getTrakt().users().updateList(UserSlug.ME, String.valueOf(TEST_LIST_WITH_ITEMS_TRAKT_ID), list));
	...
	public void test_listItems() throws IOException {
		List<ListEntry> entries = executeCall(getTrakt().users().listItems(UserSlug.ME, String.valueOf(TEST_LIST_WITH_ITEMS_TRAKT_ID), null));
	...
	public void test_addListItems() throws IOException {
		...
		// add items...
		SyncResponse response = executeCall(getTrakt().users().addListItems(UserSlug.ME, String.valueOf(TEST_LIST_WITH_ITEMS_TRAKT_ID), items));
		...
		response = executeCall(getTrakt().users().deleteListItems(UserSlug.ME, String.valueOf(TEST_LIST_WITH_ITEMS_TRAKT_ID), items));
	...
	public void test_reorderListItems() throws IOException {
		...
		ListReorderResponse response = executeCall(
				getTrakt().users().reorderListItems(UserSlug.ME, String.valueOf(TEST_LIST_WITH_ITEMS_TRAKT_ID), ListItemRank.from(newRank)));
	...
	public void test_historyEpisodesAndMovies() throws IOException {
		List<HistoryEntry> history = executeCall(getTrakt().users().history(TestData.USER_SLUG, 1, DEFAULT_PAGE_SIZE, null, null, null));
	...
	public void test_historyEpisodes() throws IOException {
		List<HistoryEntry> history = executeCall(getTrakt().users().history(TestData.USER_SLUG, HistoryType.EPISODES, 1, DEFAULT_PAGE_SIZE, null, null, null));
		for (HistoryEntry entry : history) {
			assertThat(entry.id).isGreaterThan(0);
			assertThat(entry.watched_at).isNotNull();
			assertThat(entry.action).isNotEmpty();
	...
	public void test_historyMovies() throws IOException {
		List<HistoryEntry> history = executeCall(getTrakt().users().history(UserSlug.ME, HistoryType.MOVIES, 1, DEFAULT_PAGE_SIZE, null, null, null));
	...
	public void test_historyItem() throws IOException {
		List<HistoryEntry> history = executeCall(
				getTrakt().users().history(UserSlug.ME, HistoryType.MOVIES, TestData.MOVIE_WATCHED_TRAKT_ID, 1, DEFAULT_PAGE_SIZE, null,
						OffsetDateTime.of(2016, 8, 3, 9, 0, 0, 0, ZoneOffset.UTC), OffsetDateTime.of(2016, 8, 3, 10, 0, 0, 0, ZoneOffset.UTC)));
	...
	public void test_ratingsMovies() throws IOException {
		List<RatedMovie> ratedMovies = executeCall(getTrakt().users().ratingsMovies(TestData.USER_SLUG, RatingsFilter.ALL, null));
		assertRatedEntities(ratedMovies);
	}

	@Test
	public void test_ratingsMovies_filtered() throws IOException {
		List<RatedMovie> ratedMovies = executeCall(getTrakt().users().ratingsMovies(TestData.USER_SLUG, RatingsFilter.TOTALLYNINJA, null));
	...
	public void test_ratingsShows() throws IOException {
		List<RatedShow> ratedShows = executeCall(getTrakt().users().ratingsShows(TestData.USER_SLUG, RatingsFilter.ALL, null));
		assertRatedEntities(ratedShows);
	}

	@Test
	public void test_ratingsSeasons() throws IOException {
		List<RatedSeason> ratedSeasons = executeCall(getTrakt().users().ratingsSeasons(TestData.USER_SLUG, RatingsFilter.ALL, null));
		assertRatedEntities(ratedSeasons);
	}

	@Test
	public void test_ratingsEpisodes() throws IOException {
		List<RatedEpisode> ratedEpisodes = executeCall(getTrakt().users().ratingsEpisodes(TestData.USER_SLUG, RatingsFilter.ALL, null));
		assertRatedEntities(ratedEpisodes);
	}

	@Test
	public void test_watchlistMovies() throws IOException {
		List<BaseMovie> movies = executeCall(getTrakt().users().watchlistMovies(UserSlug.ME, null));
	...
	public void test_watchedMovies() throws IOException {
		List<BaseMovie> watchedMovies = executeCall(getTrakt().users().watchedMovies(TestData.USER_SLUG, null));
		assertSyncMovies(watchedMovies, "watched");
	}

	@Test
	public void test_watchedShows() throws IOException {
		List<BaseShow> watchedShows = executeCall(getTrakt().users().watchedShows(TestData.USER_SLUG, null));
